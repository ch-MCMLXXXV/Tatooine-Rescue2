...
ordersRouter.post('/', requireUser, async (req, res, next) => {
	const { userId, purchaseComplete, adoption_fee, productId, quantity } =
		req.body;
	const orderData = {
		id: req.user.id,
		userId,
...
ordersRouter.get('/admin/all', requireUser, async (req, res, next) => {
	try {
...
ordersRouter.get('/user/history', requireUser, async (req, res, next) => {
	try {
